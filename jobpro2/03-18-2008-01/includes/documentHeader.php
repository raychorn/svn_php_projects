<?php
	...
	function encryptData($value, $key = "ilovetofuckteensintheassalldaylong") {
		$key = substr($key,0,32);
		$text = $value;
		$iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB);
		$iv = mcrypt_create_iv($iv_size, MCRYPT_RAND);
		$crypttext = mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $key, $text, MCRYPT_MODE_ECB, $iv);
		return asc2hex($crypttext);
	}
	
	function decryptData($value, $key = "ilovetofuckteensintheassalldaylong") {
		$key = substr($key,0,32);
		$crypttext = hex2asc($value);
		$iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB);
		$iv = mcrypt_create_iv($iv_size, MCRYPT_RAND);
	...
	class MSSQLDB {
		...
		// Constructor
		function __construct($inDatabaseName, $serverName = "", $username = "", $userPassword = "", $dbName = "SQL2005") {
			...
			if (strlen($dbName) > 0) {
				$this->dbVendor = (($dbName == "SQL2005") ? $dbName : (($dbName == "MYSQL") ? $dbName : ""));
			}
			$this->isMySQL = (($this->dbVendor == "MYSQL") ? true : false);
			$this->db_connection = (($this->isMySQL) ? mysql_connect($this->serverName, $this->username, $this->userPassword) : mssql_connect($this->serverName, $this->username, $this->userPassword));
			...
			$retVal = (($this->isMySQL) ? mysql_select_db($inDatabaseName) : mssql_select_db($inDatabaseName));
			if ($retVal == false) {
				$this->status = -101;
		...
		function query_database($inQuery) {  // Generic query function
			...
			$query_result = (($this->isMySQL) ? mysql_query($inQuery, $this->db_connection) : mssql_query($inQuery, $this->db_connection));
			...
			if ($this->isQueryResult($query_result)) {
				...
				while ($row = (($this->isMySQL) ? mysql_fetch_object($query_result) : mssql_fetch_object($query_result))) {
					 $result[] = $row;
				}
				
				if ($this->isQueryResult($query_result)) {
					(($this->isMySQL) ? mysql_free_result($query_result) : mssql_free_result($query_result));	// dispose of the query
				}
				...
				$query = (($this->isMySQL) ? "select LAST_INSERT_ID() as 'last_insert_id'" : "select SCOPE_IDENTITY() AS last_insert_id");	// get the last insert id
				$query_result = (($this->isMySQL) ? mysql_query($query) : mssql_query($query));
				if ($query_result == false) {
					$this->status = -201;
					$this->statusMsg = 'Query for identity after insert failed: '.$inQuery . ' (' . var_print_r($query_result) . ')' . ', (' . $this->isQueryResult($query_result) . ')';
					return;
				}
													 
				$query_result = (($this->isMySQL) ? mysql_fetch_object($query_result) : mssql_fetch_object($query_result));
				
				if ($this->isQueryResult($query_result)) {
					(($this->isMySQL) ? mysql_free_result($query_result) : mssql_free_result($query_result));
				}
...
//	echo getcontent("www.myhost.com", "81", "/"));
	...
	function var_dump_array($mixed = null) {
		$content = '(ILLEGAL VAR)';
		if (is_array($mixed)) {
			$content = '';
			foreach ($mixed as $key => $value) {
				$content .= $key . '=' . var_print_r($value) . '<br>';
			}
		}
		return $content;
	}
	...
	function serverHTTPHost() {
		...
		array_pop($ar);
		$p = join('/', $ar);
		$_SERVER["HTTP_HOST"] = 'http' . (($_SERVER["SERVER_PORT"] == 443) ? 's' : '') . '://' . $_SERVER["SERVER_NAME"] . '/' . $p;
